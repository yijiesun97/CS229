...
def grid_search(Cs):
    ...
    for i in range(len(Cs)):
        clf = LogisticRegressionCV(Cs=[Cs[i]], cv=10, solver='newton-cg', penalty='l2', max_iter=500, tol=1e-3,
...
Cs_fine = np.linspace(1e-1, 1e+1, 10)
grid_search(Cs_fine)

C_best = Cs_fine[5]
print("Optimal C:", 1/C_best)

## logistic regression
logisticRegr = LogisticRegression(penalty='l2', C=C_best, solver='liblinear', class_weight="balanced", tol=1e-4,
